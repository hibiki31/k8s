...
作業順は **tk8s-cp2 / node002 の事前確認 → 登録 → PXE → 起動後確認** とし、合格後に cp3、ワーカー 3 台へ進む。以下は 2026-09-23 に整理した手順。後続の提供ログで cp2 の起動前構成・MAC 登録と INSTALLING、さらに [UP 表示](#2026-09-23-node002-の-up-表示確認) を確認したため、現在は **6.3 の同期ログ・SSH・OS 内部確認** へ進む。登録・VM 起動は再実行しない。node001 の確認済み結果は [ホスト名による SSH と OS 内部の確認](#2026-09-23-ホスト名による-ssh-と-os-内部の確認) を引き継ぎ、再展開しない。VM・MAC・IP の正本は [環境構成](environment.md#2026-09-22-の稼働後確認)、容量は [VM 一覧](vm-list.md) を参照する。
...
証跡は `input/60-install-by-pxe/2026-09-23-node002-up-redacted.txt`。チャットのコマンドと一覧を転記し、実ヘッド名は `<HEADNODE>` に匿名化した。UP は確認済みだが、同期ログ・SSH・OS 内部・外部通信の出力はまだ共有されていない。R-01 全体の完了とは扱わず、次は [6.3 の確認](#63-cp2-の起動後を確認して-cp3-へ進む) を行う。BCM ヘッドで `cmsh -c 'device synclog node002'` と `ssh root@node002`、接続先で手順 5 と 6.3 の確認コマンドを実行する案内であり、これらは今回未実行の例。
