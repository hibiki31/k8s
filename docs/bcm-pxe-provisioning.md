...
以下は追加ディスクを初期化せず、OS のみ BCM で展開する既存方式。BCM の初回 FULL で TopoLVM 用 PV / VG も作成する場合は、後続の [ワーカー LVM 設計案](worker-storage.md) を先に参照する。VG + 最小 LV の初回作成用と、データディスクを再作成対象から外す運用用のレイアウトを分ける提案であり、まだ適用していない。
