...
| ゲストの IP・ゲートウェイ・DNS 設定 | ヘッド・cp1 実機確認、cp2 提供ログ確認 | node002 の内部 MAC 登録に加え、ゲストの予定 IP・ヘッド経由の経路・外部名前解決も確認。ヘッドと cp1 の結果は既存記録を引き継ぐ。残り 4 台は MAC 未登録。ワーカー定義の IP は予約値と不一致。[構成](environment.md) |
...
| R-01: BCM による 6 台への OS 展開 | 2 / 6 台で UP・OS 内部・外部 HTTPS を確認 | node001 と node002 は UP・SSH・ディスク上の XFS ルート・失敗サービスなし・外部 HTTPS を確認済み。node002 の同期ログ・実際の展開モードの証跡は未共有。node003～node006 は MAC 未登録・DOWN / unassigned。[node002 の実施記録](bcm-pxe-provisioning.md#2026-09-23-node002-の-ssh-と-os-内部の確認) |
...
1. cp2 の [SSH・OS 内部・外部 HTTPS 確認](bcm-pxe-provisioning.md#2026-09-23-node002-の-ssh-と-os-内部の確認) は完了。同期ログの証跡を補完し、[手順 6.1～6.3](bcm-pxe-provisioning.md#61-cp2-の接続ディスク起動設定を確認する) の対象を cp3 / node003 に置き換えて事前確認・登録・展開へ進む。ヘッドの永続 NIC 構成は今回も未確認。
