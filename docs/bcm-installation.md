...
本書の「確認」は保存画面の読み取りを意味する。今回、VM やクラスタへの接続・設定変更・インストールの再実行は行っていない。入力途中の値、Summary、Show config を区別し、後続画面で確認できる値を記録する。設定ファイルの原本や実行ログはなく、既定値と利用者が変更した値の区別は、画面間の変化がある箇所に限る。
