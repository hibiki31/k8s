...
本書は初期資料 `docs/vm-list.md` と `env.md` の環境情報、2026-09-22 に利用者から提供されたネットワーク構成、および同日の BCM インストーラー画面を整理したものです（整理日: 2026-09-22）。提供情報で作成済みとされた事項、画面で確認できる入力・構成内容、今後の計画を区別しています。今回の文書整理では実環境への接続・再検証は行っていません。VM ごとの割り当ては [VM 一覧](vm-list.md)、実施状況は [検証状況](validation.md) を参照してください。
