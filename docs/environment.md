...
- `domblklist tk8s-cp1 --details` は `file / disk / vda` の 1 台で、対応する VM イメージ名を確認しました。物理ホスト上の親パスは非掲載。容量 128 GiB、UEFI・Secure Boot・PXE 起動順序は今回の出力にはなく、以前の構成記録を引き継ぐ範囲です。

次は cp1 の容量・起動設定を確認し、停止中に `node001` の MAC 登録と OS ディスク限定を行います。ワーカーのディスク識別と残りの VM の実接続照合は別途必要です。物理ホスト名・ユーザー名・ネットワーク UUID・ブリッジの MAC は公開文書へ転記しません。
