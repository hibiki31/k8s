...
本書は初期資料 `docs/vm-list.md` と `env.md` の環境情報、2026-09-22 に利用者から提供されたネットワーク構成、同日の BCM インストーラー画面と稼働後の読み取り専用調査を整理したものです（整理日: 2026-09-22）。以下の計画・画面確認の記述は当時の記録として保持し、実測との差は [稼働後確認](#2026-09-22-の稼働後確認) に記載します。VM ごとの割り当ては [VM 一覧](vm-list.md)、実施状況は [検証状況](validation.md) を参照してください。
...
**仮想ネットワークと NIC 接続は作成済みとの提供情報です。ヘッドノードの IP とネットワーク範囲はインストーラーの入力・構成表示まで確認できましたが、稼働後の設定反映や疎通は未確認です。各ノードの IP は引き続き設定計画・予約値として保持します。** 作成済み構成も今回実環境で再確認したものではなく、設定反映や疎通の確認結果は [検証状況](validation.md) で管理します。
...
以下は作成済み NIC 接続として提供された対応表です。

| VM 名 | 接続ネットワーク | MAC アドレス |
|---|---|---|
| `tk8s-bcm` | `tk8s-external` | `52:54:00:64:00:10` |
| `tk8s-bcm` | `tk8s-internal` | `52:54:00:65:00:01` |
...
これらの画面には MAC アドレスがないため、上の作成済み NIC 接続表とゲスト内インターフェイス名の照合は未確認です。稼働後に MAC を照合して実接続と IP を確認します。画面の論理的な対応だけで配線や疎通の確認済みとは扱いません。
...
BCM ヘッドノード上で同日 22 時台（JST）に読み取り専用で確認しました。証跡は `input/40-bcm-pxe/2026-09-22-headnode-check.txt`。物理ホストの libvirt 構成と計算ノードは未確認です。以下は既存計画の上書きではなく実測との比較です。
...
稼働中の NIC は次の対応です。上の利用者提供 MAC 表とは内部・外部の対応が逆です。ただし、外部ゲートウェイへの ping は成功しています。実際の仮想ネットワーク接続先の変更有無を確認し、IP 設定ミスと即断しません。
...
既存の BCM ノード定義と今後の VM 対応案は以下です。**全台 MAC 未登録のため、VM 対応はまだ確定・適用されていません。** 内部 MAC と目標 IP は上の [IP 計画](#ip-アドレス計画)・[MAC 表](#nic-と-mac-アドレスの対応) を正とし、実際の libvirt 設定と照合して登録します。

| 現在の BCM 定義 | 現在の IP | 対応させる VM（計画） | 次の対応 |
|---|---|---|---|
| `node001` | `172.30.65.11` | `tk8s-cp1` | MAC 登録、最初の 1 台として展開 |
| `node002` | `172.30.65.12` | `tk8s-cp2` | 最初の 1 台の確認後に MAC 登録・展開 |
