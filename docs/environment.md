...
以下のネットワーク表は構成計画とインストーラー画面の記録です。ヘッドの稼働後の IP と NIC は [実機確認](#2026-09-22-の稼働後確認)、BCM / cp1 の実接続は [後続の libvirt 出力](#仮想化ホスト側の接続確認) で照合しました。**計算ノードの IP 表は設定計画・予約値です。node001～003 は 2026-09-23 の [直接確認](#2026-09-23-node001003-の直接確認) で UP・SSH・予定 IP と外部 HTTPS を確認しました。ワーカー 3 台は未確認です。** 進捗は [検証状況](validation.md) で管理します。
...
以下は作成済み NIC 接続として提供された対応表です。BCM の 2 行は、同日後続の [libvirt 出力](#仮想化ホスト側の接続確認) に基づき当初資料から訂正しました。cp1 の内部 MAC も同出力で照合済みです。cp2 は [9 月 23 日の提供ログ](#2026-09-23-cp2-の起動前構成確認)、cp3 は [後続の NIC 照会](#2026-09-23-cp3-の-nic-と-bcm-登録確認) で照合しました。ワーカーは当初の提供情報を保持しています。
...
実デバイス名・StorageClass 名は未記録です。9 月 23 日に [ワーカーの LVM 設計案](worker-storage.md) を作成し、追加ディスクに `vg_topolvm` と未マウントの `bcm_seed` 4 MiB を作る XML を静的検証しました。VG / LV 名は提案値で、未設定です。実作業時に OS 用ディスクと追加ディスクの対応を確認してから確定します。
...
既存の BCM ノード定義と VM 対応は以下です。9 月 22 日の利用者操作で **`node001` と `tk8s-cp1` の MAC 対応は登録済み**。同日の再照会では残る 5 台は未登録でした。9 月 23 日の後続共有で node002・node003 の MAC 登録も確認したため、表の次の対応を更新しています。内部 MAC と目標 IP は上の [IP 計画](#ip-アドレス計画)・[MAC 表](#nic-と-mac-アドレスの対応) を正とし、実際の libvirt 設定と照合して登録します。

| 現在の BCM 定義 | 現在の IP | 対応させる VM（計画） | 次の対応 |
|---|---|---|---|
| `node001` | `172.30.65.11` | `tk8s-cp1` | UP・同期完了・SSH・OS 内部・外部 HTTPS を直接確認。[結果と残課題](#2026-09-23-node001003-の直接確認) |
| `node002` | `172.30.65.12` | `tk8s-cp2` | UP・同期完了・SSH・OS 内部・外部 HTTPS を直接確認。[結果と残課題](#2026-09-23-node001003-の直接確認) |
| `node003` | `172.30.65.13` | `tk8s-cp3` | UP・同期完了・SSH・OS 内部・外部 HTTPS を直接確認。[結果と残課題](#2026-09-23-node001003-の直接確認) |
| `node004` | `172.30.65.14` | `tk8s-worker1` | MAC 登録・展開、IP を計画の `.21` に変更 |
| `node005` | `172.30.65.15` | `tk8s-worker2` | MAC 登録・展開、IP を計画の `.22` に変更 |
| `node006` | `172.30.65.16` | `tk8s-worker3` | MAC 登録・展開、IP を計画の `.23` に変更 |
...
## 2026-09-23: cp3 の NIC と BCM 登録確認

ユーザーが物理ホストで実行した `virsh domiflist tk8s-cp3` の提供出力は、virtio NIC 1 本、`tk8s-internal`、上の MAC 表と同じ内部 MAC を示す。`--inactive` なしの照会であり、永続構成との比較は未実施。Interface 欄の `-` だけで VM の停止状態を断定しない。

続いて BCM ヘッドでの MAC 登録と `device list` により、node003 に同じ MAC、`172.30.65.13`、`default` / `internalnet` が表示されることを確認した。これは BCM 定義の確認で、ゲスト内部の IP・NIC 名・経路は未確認。ディスク・CPU / メモリ・UEFI の実設定も今回は共有されておらず、[VM 一覧](vm-list.md) の計画・初期記録と区別する。

受領日は 2026-09-23（JST）、ユーザーの実行日時・終了コード・物理ホストの製品版は未記録。証跡は `input/60-install-by-pxe/2026-09-23-node003-registration-up-redacted.txt`。Codex は実環境で再照会していない。操作順と UP の確認範囲は [実施記録](bcm-pxe-provisioning.md#2026-09-23-node003-の-nicmac-登録と-up-確認)、現在の進捗は [検証状況](validation.md) を参照。物理ホスト名・ユーザー名・実ヘッド名は匿名化した。
...
Codex が BCM ヘッドから 01:21～01:25 JST に照会・SSH で確認した最新の実測。上記の個別記録はそれぞれ採取時点の結果として保持する。コマンド・失敗と再試行・証跡は [実施記録](bcm-pxe-provisioning.md#2026-09-23-node001003-の-bcm-からの一括確認) を参照。

| 項目 | node001 | node002 | node003 |
|---|---|---|---|
| BCM / 同期ログ | UP / FULL 完了 | UP / FULL 完了 | UP / FULL 完了 |
| root SSH | 既存ホスト鍵照合・公開鍵認証 | 既存ホスト鍵照合・公開鍵認証 | イメージ公開鍵照合・公開鍵認証 |
| enp1s0 / IPv4 | `172.30.65.11/24` | `172.30.65.12/24` | `172.30.65.13/24` |
| 内部 MAC | `52:54:00:65:00:02` | `52:54:00:65:00:03` | `52:54:00:65:00:04` |
| OS / カーネル | Ubuntu 24.04.4 LTS / `6.8.0-106-generic` | 同左 | 同左 |
| CPU / メモリ | 4 CPU / free 表示 15 GiB | 同左 | 同左 |
| ディスク / ルート | vda 128 GiB / `/dev/vda3` XFS | 同左 | 同左 |
| パーティション | EFI 100 MiB、swap 16 GiB、XFS 約 111.9 GiB | 同左 | 同左 |
| swap / fstab | vda2 有効、使用 0B / UUID 形式の swap 行あり | 同左 | 同左 |
| BCM agent / failed | cmd active / 0 件 | 同左 | 同左 |
| 時刻 / 起動方式 | NTP 同期 yes、Asia/Tokyo / UEFI | 同左 | 同左 |
| 外部 DNS / HTTPS | 成功 / HTTP 200 | 同左 | 同左 |

3 台ともデフォルト経路・DNS は `172.30.65.1`。ヘッドから各ノードへの ping は 2/2 成功。ゲスト内 `nproc` / `free` の表示は 4 vCPU / 16 GiB の割り当て計画と整合するが、libvirt の永続割り当てを再照会したものではない。UEFI は `/sys/firmware/efi` の存在を確認した範囲で、実効 Secure Boot は未確認。

3 台とも展開元イメージと同一の ED25519 ホスト鍵を提示した。node003 は最初に未登録鍵で停止し、イメージ公開鍵を一時 known_hosts に固定して検証付きで再接続した。共有鍵のためノード固有鍵による識別とは区別する。既存 known_hosts・鍵・構成は変更していない。Kubernetes 構築・再起動後の swap 無効化とワーカーの展開は引き続き未確認。
