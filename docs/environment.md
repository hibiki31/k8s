...
以下のネットワーク表は構成計画とインストーラー画面の記録です。ヘッドの稼働後の IP と NIC は [実機確認](#2026-09-22-の稼働後確認)、BCM / cp1 の実接続は [後続の libvirt 出力](#仮想化ホスト側の接続確認) で照合しました。**計算ノードの IP は引き続き設定計画・予約値であり、OS 展開・疎通は未確認です。** 進捗は [検証状況](validation.md) で管理します。
...
|---|---|---|---|
| `node001` | `172.30.65.11` | `tk8s-cp1` | MAC 登録済み。起動設定・swap 方針確認後、最初の 1 台として展開 |
...
後続の登録・swap 調査の証跡は `input/40-bcm-pxe/2026-09-22-cp1-swap-review.txt`。`node001` の MAC は `52:54:00:65:00:02`、BOOTIF は `172.30.65.11` / `internalnet`、状態は `DOWN` と再確認しました。カテゴリ継承の swap 16G は変更されていません。次は cp1 の起動設定を確認し、[swap の扱い](bcm-pxe-provisioning.md#3-os-ディスクと-swap-の扱いを確認する) を選んで初回展開へ進みます。ワーカーのディスク識別と残りの VM の実接続照合は別途必要です。物理ホスト名・ユーザー名・ネットワーク UUID・ブリッジの MAC は公開文書へ転記しません。
