...
以下のネットワーク表は構成計画とインストーラー画面の記録です。ヘッドの稼働後の IP と NIC は [実機確認](#2026-09-22-の稼働後確認)、BCM / cp1 の実接続は [後続の libvirt 出力](#仮想化ホスト側の接続確認) で照合しました。**計算ノードの IP 表は設定計画・予約値です。node001 は 2026-09-23 に予定 IP での展開・UP を確認し、残り 5 台の展開と各ノードの外部疎通は未確認です。** 進捗は [検証状況](validation.md) で管理します。
...
以下は作成済み NIC 接続として提供された対応表です。BCM の 2 行は、同日後続の [libvirt 出力](#仮想化ホスト側の接続確認) に基づき当初資料から訂正しました。cp1 の内部 MAC も同出力で照合済みです。他のノードは当初の提供情報を保持しています。
...
既存の BCM ノード定義と今後の VM 対応案は以下です。同日後続の利用者操作で **`node001` と `tk8s-cp1` の MAC 対応は登録済み**。ヘッド上で再照会し、残る 5 台は未登録であることを確認しました。 内部 MAC と目標 IP は上の [IP 計画](#ip-アドレス計画)・[MAC 表](#nic-と-mac-アドレスの対応) を正とし、実際の libvirt 設定と照合して登録します。

| 現在の BCM 定義 | 現在の IP | 対応させる VM（計画） | 次の対応 |
|---|---|---|---|
| `node001` | `172.30.65.11` | `tk8s-cp1` | 9 月 23 日に初回展開・UP 確認。OS 内部・外部疎通を確認 |
...
利用者の PXE 画像・`device list` 出力と、00:10～00:11 JST のヘッド上の再照会に基づきます。手順・画像ごとの確認範囲・証跡は [初回展開記録](bcm-pxe-provisioning.md#2026-09-23-node001-の初回-pxeos-展開記録) を参照。
...
| 未確認 | ルートのマウント元、実パーティション、swap、SSH ログイン、外部疎通。SSH は未登録ホスト鍵の検証で停止 |
